...
import { useGetExclusiveRightsLicense, useUploadExclusiveRightsLicense, useDeleteExclusiveRightsLicense } from '../hooks/useQueries';
...
export default function ExclusiveRightsLicenseManagement() {
  const { data: currentLicense, isLoading } = useGetExclusiveRightsLicense();
  const uploadLicense = useUploadExclusiveRightsLicense();
  const deleteLicense = useDeleteExclusiveRightsLicense();
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      link.href = url;
      link.download = 'Exclusive-Rights-License.pdf';
  ...
  return (
    <Card className="bg-card/50 backdrop-blur-sm border-border/40">
      <CardHeader>
        ...
        <CardDescription>
          Manage the license document that will be automatically included with all Exclusive Right ZIP file downloads
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-6">
        ...
        {isLoading ? (
          ...
          <Alert className="border-green-200 dark:border-green-800 bg-green-50 dark:bg-green-950">
            ...
            <AlertDescription className="text-sm text-green-900 dark:text-green-100">
              <strong>License Active:</strong> The Exclusive Rights License Agreement is currently uploaded and will be automatically included in all Exclusive Right ZIP file downloads.
            </AlertDescription>
          </Alert>
        ) : (
          <Alert className="border-yellow-200 dark:border-yellow-800 bg-yellow-50 dark:bg-yellow-950">
            ...
            <AlertDescription className="text-sm text-yellow-900 dark:text-yellow-100">
              <strong>No License Uploaded:</strong> Upload a license document to automatically include it in Exclusive Right ZIP downloads.
            </AlertDescription>
          </Alert>
        )}
        ...
        <Alert className="border-primary/30 bg-primary/5">
          ...
          <AlertDescription className="text-sm">
            <strong>How it works:</strong> When a user purchases or downloads an Exclusive Right beat with ZIP delivery, 
        ...
        <div className="space-y-3 p-4 border border-border/40 rounded-lg bg-background/50">
          ...
          <div className="space-y-2">
            <input
              type="file"
              id="exclusive-license-upload"
              accept=".pdf,.doc,.docx,.txt"
              onChange={handleFileUpload}
              disabled={uploading}
              className="hidden"
            />
            <label htmlFor="exclusive-license-upload">
              <Button
                asChild
                disabled={uploading}
        ...
        <div className="text-sm text-muted-foreground space-y-3 p-4 border border-border/40 rounded-lg bg-background/50">
          ...
          <ul className="list-disc list-inside space-y-2 ml-2">
            <li>The license document is automatically included in all Exclusive Right ZIP file downloads</li>
      ...
      <AlertDialog open={showDeleteDialog} onOpenChange={setShowDeleteDialog}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This will remove the Exclusive Rights License Agreement from the system. 
              Future Exclusive Right ZIP downloads will not include a license document until you upload a new one.
