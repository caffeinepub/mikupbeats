...
import { useGetBasicRightsLicense, useUploadBasicRightsLicense, useDeleteBasicRightsLicense } from '../hooks/useQueries';
...
export default function BasicRightsLicenseManagement() {
  const { data: currentLicense, isLoading } = useGetBasicRightsLicense();
  const uploadLicense = useUploadBasicRightsLicense();
  const deleteLicense = useDeleteBasicRightsLicense();
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      link.href = url;
      link.download = 'MvA-Basic-Rights-License.pdf';
    ...
    <Card className="bg-card/50 backdrop-blur-sm border-border/40">
      <CardHeader>
        <CardTitle className="flex items-center gap-2">
          ...
          M.v.A Basic Rights License Agreement
        </CardTitle>
        <CardDescription>
          Manage the license document that will be automatically included with all Basic Right ZIP file downloads
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-6">
        ...
        {isLoading ? (
          ...
          <Alert className="border-green-200 dark:border-green-800 bg-green-50 dark:bg-green-950">
            ...
            <AlertDescription className="text-sm text-green-900 dark:text-green-100">
              <strong>License Active:</strong> The M.v.A Basic Rights License Agreement is currently uploaded and will be automatically included in all Basic Right ZIP file downloads.
            </AlertDescription>
          </Alert>
        ) : (
          <Alert className="border-yellow-200 dark:border-yellow-800 bg-yellow-50 dark:bg-yellow-950">
            ...
            <AlertDescription className="text-sm text-yellow-900 dark:text-yellow-100">
              <strong>No License Uploaded:</strong> Upload a license document to automatically include it in Basic Right ZIP downloads.
            </AlertDescription>
          </Alert>
        )}
        ...
        <Alert className="border-primary/30 bg-primary/5">
          ...
          <AlertDescription className="text-sm">
            <strong>How it works:</strong> When a user purchases or downloads a Basic Right beat with ZIP delivery, 
        ...
        <div className="text-sm text-muted-foreground space-y-3 p-4 border border-border/40 rounded-lg bg-background/50">
          ...
          <ul className="list-disc list-inside space-y-2 ml-2">
            <li>The license document is automatically included in all Basic Right ZIP file downloads</li>
      ...
      <AlertDialog open={showDeleteDialog} onOpenChange={setShowDeleteDialog}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This will remove the M.v.A Basic Rights License Agreement from the system. 
              Future Basic Right ZIP downloads will not include a license document until you upload a new one.
