 ...
 *
 * @example
 * // URL: https://app.com/#/dashboard?beatId=123&rightsType=basic&session_id=xyz
 * // After removeQueryParams(['beatId', 'rightsType', 'session_id'])
...
export function removeQueryParams(paramNames: string[]): void {
    ...
    if (hash && hash.length > 1) {
        ...
        if (queryStartIndex !== -1) {
            ...
            // Parse and remove the specified parameters
            const params = new URLSearchParams(queryString);
